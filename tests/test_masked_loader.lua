...
MAX_ITER = 10
print('Testing...')
print('checking minLabelToOne method')
local loader0 = SequentialDB('./tests/toydata2.h5', 1, false)
data, labels = loader0:getBatch()
assert(labels:min() == 0)
assert(not loader0:minLabelToOne())
assert(loader0:getLabelWeights():eq(torch.FloatTensor({0.5,0.5})):all())
loader0:reset()
data, labels = loader0:getBatch()
assert(labels:min() == 1)
local loader1 = SequentialDB('./tests/toydata3.h5', 1, false)
assert(loader1:minLabelToOne())
